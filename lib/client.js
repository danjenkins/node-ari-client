  ...
  return new Promise(function(resolve, reject) {
    ...
    function swaggerLoaded () {
      if(self._swagger.ready === true) {
        ...
        if (self.proxy) {
          self._proxy = new Amqp('amqp://guest:guest@docker.local');
          self._proxy.init(function () {
            resolve(self);
          });
          return;
        }
    ...
    function attachResource (resource) {
      ...
      function attachOperation (operation) {
        self[resource][operation] = callSwaggerClient;
         ...
         *  @returns {Q} promise - a promise that will resolve to a client
         */
        function callSwaggerClient (/* args..., callback */) {
          ...
          return new Promise(function(innerResolve, innerReject) {
            ...
            if (self._client._proxy) {
              console.log('Sending a command via proxy CLIENT');
              var info = self._swagger.apis[self._resource].operations[operation];

              var dialogId = 'foo';

              self._proxy.send(dialogId, info.path, info.method, options, processResponse, swaggerError);
              return;
            }
...
Client.prototype.start = function (apps, subscribeAll, callback) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (self._proxy) {
      self._proxy.start((!_.isArray(apps)) ? [apps] : apps, function () {
        resolve();
        self._proxy.on('events', processMessage);
      })
      return;
    }
    ...
    function processMessage (msg, flags) {
      ...
      _.each(eventModel.properties, function (prop) {
        if (_.contains(_resources.knownTypes, prop.dataType) &&
          ...
          instance.proxyDialogId = event.proxyDialogId || null;
...
Client.prototype.stop = function () {
  ...
  if (self.proxy) {
    console.log('STOP and disconnect from AMQP');
  }
...
module.exports.connectProxy = function (amqpURI,
    /**
     *  @callback connectCallback
     *  @memberof module:ari-client
     *  @param {Error} err - error object if any, null otherwise
  ...
  var client = new Client(amqpURI, 'asterisk', 'asterisk');
  client.proxy = true;
  client.setMaxListeners(0);
